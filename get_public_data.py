        ...
        while position > 0:
            ...
            if b'\n' in buffer:
                ...
                try:
                    ...
                    print("Erreur : la dernière ligne n'est pas un format JSON valide.")
                    return None
...
def is_first_date_more_recent(date1, date2):
    if not isinstance(date1, str) or not isinstance(date2, str):
        raise ValueError("Les dates doivent être des chaînes de caractères.")
...
def get_candles(MARKET,res):
    ...
    file_name = f'{directory}/CANDLES_{res}.json'
    last_line = read_last_line(file_name)
    #print("Type de last_line:", type(last_line))

    candles.reverse()
    #print(f'last_line = {last_line}')
    date2 = last_line['startedAt']

    for candle in candles:
        candle.pop("orderbookMidPriceOpen", None)
        candle.pop("orderbookMidPriceClose", None)
        date1 = candle['startedAt']

        if is_first_date_more_recent(date1,date2):
            add_to_json_file(file_name, candle)
